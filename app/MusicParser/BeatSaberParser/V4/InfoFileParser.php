...
class InfoFileParser extends AbstractInfoFileParser
{
	...
	public function parse(): array
	{
		...
		foreach ($data['difficultyBeatmaps'] as $difficultyBeatmapIndex => $difficultyBeatmap) {
			$filename = $difficultyBeatmap['beatmapDataFilename'];
			$difficultyFileData = $this->loadAndParseDifficultyFile($filename);

			$data['difficultyBeatmaps'][$difficultyBeatmapIndex]['beatmapFileData'] = $difficultyFileData;
		}

		// Load audio
		$audioFilename = $this->infoFileData['audio']['songFilename'];
		...
		// Load cover image
		$coverImageFilename = $this->infoFileData['coverImageFilename'];
